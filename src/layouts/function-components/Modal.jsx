...
const Modal = ({ modal_content_data, modalStateForm, setModalStateForm, confirmStateForm, setConfirmStateForm, onConfirm}) => {
    ...
    const handleClickOutsideDropdown =(e)=>{
        if(modalStateForm && !anchorRef.current?.contains(e.target)){
    ...
    <>
    {modalStateForm &&
    <div className="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true" aria-hidden="true">
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity">
        </div>
        <div className="fixed inset-0 z-10 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div ref={anchorRef} className="border-2 border-paqariGreen relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
